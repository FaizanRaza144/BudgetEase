...
async function loadTransactions() {
    try {
        const response = await fetch("http://3.25.202.196/getTransactions", {
            method: "GET",
...
document.getElementById("editTransactionForm").addEventListener("submit", async (event) => {
    ...
    try {
        const response = await fetch(`http://3.25.202.196/updateTransactions/${transactionId}`, {
            method: "PUT",
            headers: {
...
async function deleteTransaction(event) {
    ...
    try {
        const response = await fetch(`http://3.25.202.196/delTransactions/${transactionId}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
...
async function populateCategories() {
    try {
        const response = await fetch("http://3.25.202.196/getCategories", {
            method: "GET",
            headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
